...
<#
.SYNOPSIS
  Genere Dockerfiles + compose locaux (backend/, frontend/) + compose global (include).

.NOTES
  Layout : Django dans <backend>/, Astro dans <frontend>/.
  Compose : backend/docker-compose.yml (uv sync --frozen a chaque up),
  frontend/docker-compose.yml, racine docker-compose.yml (include).
#>

function New-DockerStack {
    <#
    .SYNOPSIS
      Dockerfiles uv/Astro + compose backend, frontend (si Astro) et global.
...
'@
    ...
    Write-TextFile -Path (Join-Path $scriptsDir "docker-uv-sync.sh") -Content @'
#!/bin/sh
# Synchronise le venv avec uv.lock a chaque demarrage de conteneur.
set -e
cd /app
echo "uv sync --frozen..."
uv sync --frozen
...
'@
    ...
    if ($HasFrontend) {
        $feScriptsDir = Join-Path $Root "$FrontendDirName\scripts"
...
pnpm_install_safe() {
  if [ -f pnpm-lock.yaml ]; then
    ...
    echo "Conseil : pnpm install (puis commit pnpm-lock.yaml)."
    pnpm install
  fi
}
...
'@
        ...
        Write-TextFile -Path (Join-Path $Root "$FrontendDirName\Dockerfile") -Content @'
...
'@
        ...
        Write-TextFile -Path (Join-Path $Root "$FrontendDirName\.dockerignore") -Content @'
node_modules/
dist/
.astro/
.git/
...
'@
    ...
    Write-TextFile -Path (Join-Path $backendDir "Dockerfile") -Content @'
...
'@
    ...
    Write-TextFile -Path (Join-Path $backendDir ".dockerignore") -Content @'
.venv/
__pycache__/
*.pyc
db.sqlite3
...
'@
    ...
    Write-SplitComposeFiles -Root $Root -BackendDirName $BackendDirName `
        -FrontendDirName $FrontendDirName -PostgresHostPort $PostgresHostPort `
